...
public abstract class DateUtils {
  ...
  /*  public static Date behindDay(Date currDate,int days){
    ...
    public static Date getDateAfter(Date d, int day) {
        ...
        String afterDate = sdf.format(d);

        Calendar now = Calendar.getInstance();
        now.setTime(d);
        now.set(Calendar.DATE, now.get(Calendar.DATE) + day);
        //System.out.println("日期（"+sdf.format(d)+"）在（"+day+"）天以后是（"+sdf.format(now.getTime())+"）");
        return now.getTime();
    }


    public static Date  getAfterTime(Date  startTime ,Long hours){
        Long  start = startTime.getTime();
        Long  hoursLong = hours*60*60*1000;
        Long  newTime = start+hoursLong;
        return getDate(newTime);
    }


    public static void main(String[] args) {
     //  getDateAfter(new Date(), 180);
