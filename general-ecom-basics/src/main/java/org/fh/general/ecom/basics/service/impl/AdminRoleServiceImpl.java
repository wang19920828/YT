...
public class AdminRoleServiceImpl extends ServiceImpl<AdminRoleDao, AdminRole> implements AdminRoleService {
    ...
    public RoleListOutputDTO findPage(RoleListInputDTO dto) throws Exception {
        ...
        if(StringUtils.isNotEmpty(dto.getIsDel())){
            wrapper.eq("is_del",dto.getIsDel());
        }
        if(StringUtils.isNotEmpty(dto.getIsDisabled())){
            wrapper.eq("is_disabled",dto.getIsDisabled());
        }
        ...
        list.forEach((AdminRole temp) -> {
            ...
            Admin admin = this.adminDao.selectByPrimaryKey(temp.getCreatorId());
            po.setCreatorName(admin.getName());
            listpo.add(po);
        });
    ...
    public String addEntity(RoleAddInputDTO dto) throws Exception {
        ...
        try {
            ...
            ShouquanSaveInputDTO saveInputDTO=new ShouquanSaveInputDTO();
            saveInputDTO.setFunctions(dto.getFunctions());
            saveInputDTO.setRemark(dto.getRemark());
            saveInputDTO.setRoleId(entity.getId());
            this.saveShouquan(saveInputDTO);
    ...
    public String saveShouquan(ShouquanSaveInputDTO dto) throws Exception {
        ...
        try {
            ...
            redisHashService.delete(RedisEnum.RedisKey.QUANXIAN .getValue()+ "_" + roleId);
            redisHashService.set(RedisEnum.RedisKey.QUANXIAN.getValue() + "_" + roleId, JSONObject.toJSONString(hasFunctions), 0);
        } catch (Exception e) {
            e.printStackTrace();
            code = OutEnum.FAIL.getCode();
        }
    ...
    public Boolean selectByName(String branch,String roleName){
        ...
        AdminRole role = this.adminRoleDao.selectOne(entity);
        if(role!=null){
            return true;
        }
        return false;
