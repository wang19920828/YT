    ...
    public BaseVO addPhone(UseaddPhoneInputDTO dto) {
        ...
        try {
            ...
            if(null!=use){ //手机已绑定
                ...
                //暂时返回**************************
                baseVO.warn("该手机号已被注册!"); return baseVO;

//                //app-微信授权登录——》绑定手机号
//                if(StringUtils.isNotEmpty(one.getUnionid())){
//                    baseVO.warn("该手机号已被注册！");
//                    return baseVO;
//                }
//                //app-qq授权登录——》绑定手机号
//                if(StringUtils.isNotEmpty(one.getQquid())){
//                    baseVO.warn("该手机号已被注册！");
//                    return baseVO;
//                }
//                //app-微博授权登录——》绑定手机号
...
//                }
            ...
            if (StringUtils.isEmpty(one.getUserImg())) {
                one.setUserImg(out.getUserImg());
            }
            if (StringUtils.isEmpty(one.getNickName())) {
                one.setNickName(out.getNickName());
            }
            if (StringUtils.isEmpty(one.getSex())) {
                one.setSex(out.getSex());
            }
            //更新信息
 ...
 */
    ...
    public Boolean findIsNameByUserId(@RequestBody Long userId) {
        Boolean bo= userService.findIsNameByUserId(userId);
        return  bo;
    }
    /**
     * 手机绑定微信
     */
    @RequestMapping("USER1020")
    public BaseVO addWeixin(UserAddWeixinInputDTO dto) {
        BaseVO baseVO=new BaseVO();
        try {
            ...
            UserOutputDTO one =new UserOutputDTO();
            if(null!=use){ //已绑定
                BeanUtils.copyProperties(use,one);//one是微信
                if(StringUtils.isNotEmpty(one.getPhone())){
                    baseVO.warn("微信信息已被占用！");
                    return baseVO;
                }
            }
            //根据userId查询用户
            UserOutputDTO out = this.userService.findOne(dto.getUserId()); //out 手机
            if(out==null){
                baseVO.warn("未查到该用户！");
                return baseVO;
            }
            ...
            out.setUnionid(dto.getUnionid());
            out.setRealUnionid(dto.getUnionid());
            if(StringUtils.isEmpty(out.getUserImg())){
                out.setUserImg(dto.getUserImg());
            }
            if(StringUtils.isEmpty(out.getSex())){
                out.setSex(dto.getSex());
            }
            if(StringUtils.isEmpty(out.getNickName())){
                out.setNickName(dto.getNickName());
            }
            //绑定用户微信
            String code = userService.updateUserPhone(out);
            if(!OutEnum.SUCCESS.getCode().equals(code)){
            ...
            //删除微信用户信息
            String code1= this.userService.deleteEntityById(one.getUserId());
            if(!OutEnum.SUCCESS.getCode().equals(code1)){
                baseVO.error(OutEnum.FAIL.getMessage());
                return  baseVO;
            }
