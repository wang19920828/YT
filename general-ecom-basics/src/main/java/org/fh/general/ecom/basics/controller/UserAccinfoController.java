...
public class UserAccinfoController {
     ...
     * @param dto
     */
    @RequestMapping("UA000003")
    public BaseVO insertMinusUserAccinfo (UserAccinfoInsertInputDTO dto){
        BaseVO baseVO = new BaseVO();
        String code=userAccinfoService.insertMinusUserAccinfo(dto);
        if(!OutEnum.SUCCESS.getCode().equals(code)){
            baseVO.error(code);
            return  baseVO;
        }
